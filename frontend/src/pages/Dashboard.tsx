...
import type { Budget, ListResponse, MembershipContext, Requisition, Rfq, Supplier } from "../types";
...
export default function Dashboard() {
  ...
  const can = (permission: string) => membership.data?.permissions.includes(permission) ?? false;
  const results = useQueries({ queries: [
    { queryKey: ["requisitions"], queryFn: () => api<ListResponse<Requisition>>(session, "/api/v1/requisitions?limit=50"), enabled: can("requisitions.read") },
    { queryKey: ["suppliers"], queryFn: () => api<ListResponse<Supplier>>(session, "/api/v1/suppliers?limit=50"), enabled: can("suppliers.read") },
    { queryKey: ["rfqs"], queryFn: () => api<ListResponse<Rfq>>(session, "/api/v1/rfqs?limit=50"), enabled: can("sourcing.read") },
    { queryKey: ["budgets"], queryFn: () => api<ListResponse<Budget>>(session, "/api/v1/budgets?limit=50"), enabled: can("budgets.read") },
  ]});
  if (membership.isLoading || results.some((item) => item.isLoading)) return <Loading/>;
  if (membership.error) return <ErrorNotice error={membership.error}/>;
  const failed = results.find((item) => item.error); if (failed) return <ErrorNotice error={failed.error}/>;
  const reqs = (results[0].data as ListResponse<Requisition> | undefined)?.items || []; const suppliers = (results[1].data as ListResponse<Supplier> | undefined)?.items || []; const rfqs = (results[2].data as ListResponse<Rfq> | undefined)?.items || []; const budgets = (results[3].data as ListResponse<Budget> | undefined)?.items || [];
  ...
  return <><PageHeader eyebrow={today} title="Procurement overview" description="Work requiring attention across your organization." action={can("requisitions.write") ? <Link to="/requisitions?new=1" className="button button-primary"><Plus size={16}/>New requisition</Link> : undefined}/>
