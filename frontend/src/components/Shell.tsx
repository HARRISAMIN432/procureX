...
export default function Shell() {
  ...
  return <div className={`app-shell ${collapsed ? "nav-collapsed" : ""}`}>
    <aside className={`sidebar ${mobileOpen ? "sidebar-open" : ""}`}>
      ...
      <button className="workspace-switch" onClick={() => authMode === "oidc" && navigate("/workspaces")} disabled={authMode !== "oidc"} aria-label="Switch workspace"><Building2 size={17}/><div><small>Workspace</small><strong>{org.data?.name || "Loading…"}</strong></div>{authMode === "oidc" && <ChevronDown size={15}/>}</button>
